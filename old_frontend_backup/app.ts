...
interface TranscriptionSession {
  ...
  jbaResults?: JBAResult[];
  videoUrl?: string;
  errorMessage?: string;
}

interface AssemblyAIResults {
  id: string;
  status: string;
  text: string;
  confidence: number;
...
class SimpleTranscriptorApp {
  ...
  private setupEventListeners(): void {
    ...
    // Action buttons
    const startTranscriptionBtn = document.getElementById(
      "start-transcription"
    );
    ...
    if (forceReloadBtn) {
      forceReloadBtn.addEventListener("click", () =>
        this.forceReloadCurrentSession()
      );
    }
    ...
    if (highlightJBABtn) {
      highlightJBABtn.addEventListener("click", () =>
        this.toggleJBAHighlighting()
      );
    }
  }
  ...
  private switchView(view: string, pushState: boolean = true): void {
  ...
  private showUploadProgress(filename: string): void {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (activeSessions.length > 0) {
          console.log(
            `🔄 Found ${activeSessions.length} active sessions, starting polling...`
          );
      ...
      <div class="session-card" onclick="app.viewSession('${session.id}')">
        ...
        <div class="session-meta">
          <span><i class="fas fa-calendar"></i> ${this.formatDate(
            session.uploadTimestamp
          )}</span>
          ${
            session.transcriptionResults
              ? `<span><i class="fas fa-clock"></i> ${this.formatDuration(
                  session.transcriptionResults
                )}</span>`
              : ""
          }
          ${
            session.transcriptionResults
              ? `<span><i class="fas fa-percentage"></i> ${Math.round(
                  session.transcriptionResults.confidence * 100
                )}%</span>`
              : ""
          }
        </div>
        <div class="session-actions">
          ...
          <button class="btn btn-small btn-secondary" onclick="app.deleteSession('${
            session.id
  ...
  private renderRecentSessions(sessions: TranscriptionSession[]): void {
  ...
  private filterSessions(): void {
    ...
    const filteredSessions = this.sessions.filter(
      (session) => session.status === filterValue
    );
  ...
  public async viewSession(sessionId: string): Promise<void> {
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log(
          ...
          this.currentSession.transcriptionResults
        );
  ...
  public async forceReloadCurrentSession(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(
        `/api/transcription/${this.currentSession.id}`
      );
      const result: APIResponse = await response.json();
      ...
      if (result.success && result.data) {
        ...
        console.log(
          "🔄 Force reload - transcription results:",
          this.currentSession.transcriptionResults
        );
        ...
        console.log(
          `✅ Session reloaded - Status: ${this.currentSession?.status}`
        );
      } else {
  ...
  private renderSessionResults(): void {
    ...
    this.updateElement(
      "results-date",
      this.formatDate(session.uploadTimestamp)
    );

    if (session.transcriptionResults) {
      this.updateElement(
        "results-duration",
        this.formatDuration(session.transcriptionResults)
      );
    }
    ...
    if (session.transcriptionResults) {
      ...
      this.renderHighlights(
        session.transcriptionResults.auto_highlights_result
      );
  ...
  private renderTranscript(results: AssemblyAIResults): void {
    ...
    console.log(
      ...
      results.text ? results.text.substring(0, 200) : "NO TEXT"
    );
  ...
  private renderJBACodes(jbaCodes: JBAResult[]): void {
  ...
  private renderSpeakers(utterances: Utterance[]): void {
  ...
  private renderChapters(chapters: Chapter[]): void {
    ...
    container.innerHTML = chapters
      ...
      <div class="chapter-item" onclick="app.jumpToTimestamp(${chapter.start})">
        ...
        <p>${chapter.summary}</p>
        <small>${this.formatTimestamp(chapter.start)} - ${this.formatTimestamp(
          chapter.end
        )}</small>
      </div>
    `
  ...
  private renderHighlights(highlights?: AutoHighlights): void {
  ...
  private setupVideoForSession(videoUrl: string): void {
        ...
        if (result.success && result.data) {
          ...
          if (session.status === "completed" || session.status === "error") {
            ...
            if (session.status === "completed") {
              this.showNotification(
                `Transcription completed: ${session.filename}`,
                "success"
              );
            } else {
              this.showNotification(
                `Transcription failed: ${session.filename}`,
                "error"
              );
            }
  ...
  private async updateStats(): Promise<void> {
    try {
      ...
      if (result.success && result.data) {
        ...
        this.updateElement(
          "completed-sessions",
          stats.byStatus.completed.toString()
        );
        this.updateElement(
          "processing-sessions",
          stats.byStatus.processing.toString()
        );
      }
    } catch (error) {
  ...
  private exportTranscript(): void {
    if (!this.currentSession?.transcriptionResults) {
  ...
  private generateExportContent(): string {
    if (!this.currentSession?.transcriptionResults) return "";
  ...
  private toggleJBAHighlighting(): void {
  ...
  private formatDuration(results: AssemblyAIResults): string {
